...
local function select_candidate(candidate, idx)
...
function fuma_selector.init(env)
    ...
    page_size = (config:get_string('menu/page_size') + 0) // 1
end
...
function fuma_selector.func(key_event, env)
    ...
    if key_pressed == 'space' then
        ...
        if select_candidate(candidate, nil) == kAccepted then return kAccepted end
    end
    if key_pressed >= '1' and key_pressed <= tostring(page_size) then
        ...
        if select_candidate(candidate, selected_idx) == kAccepted then return kAccepted end
    end
    return kNoop
end
